...
function PoolCard({ pool }: { pool: (typeof BASE_FARM_POOLS)[0] }) {
...
export default function BaseFarm() {
  const { isConnected } = useAccount();
  const [filter, setFilter] = useState<"all" | "active" | "featured">("featured");

  const filteredPools = BASE_FARM_POOLS.filter((p) => {
