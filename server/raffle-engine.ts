...
export async function checkAndDrawExpiredRaffles(
  ...
  // Use Promise.all for parallel drawing of expired raffles (each is independent)
  const expiredRaffles = raffles.filter(
    r => r.status === 'active' && now > r.endTime && r.entries.length > 0
  );

  const results = await Promise.all(
    expiredRaffles.map(raffle => drawRaffleWinners(raffle, chain))
  );
