...
export function castVote(
  ...
  if (!voter || !/^0x[a-fA-F0-9]{40}$/.test(voter)) {
    throw new Error(`Invalid voter wallet address format: ${voter}`);
  }
  ...
  if (!proposal.nominees.find(n => n.id === nomineeId)) {
    throw new Error(`Invalid nominee ID: "${nomineeId}". Valid IDs: ${proposal.nominees.map(n => n.id).join(', ')}`);
  }
...
export async function checkAndFinalizeExpiredProposals(
  ...
  // Find all expired active proposals
  const expired = proposals.filter(
    p => p.status === 'active' && now > p.votingCloses
  );
  ...
  // Use Promise.all for parallel finalization (each proposal is independent)
  const results = await Promise.all(
    expired.map(proposal => finalizeProposal(proposal, emailConfig))
  );
