...
export class VRFProvider {
  ...
  async requestRandomWords(numWords: number = 1): Promise<VRFRequest> {
    ...
    if (!event) {
      throw new Error(
        `VRF requestRandomWords succeeded (tx: ${receipt.hash}) but RandomWordsRequested event not found in logs. ` +
        `This may indicate the coordinator address (${this.config.coordinatorAddress}) is incorrect or the ABI is mismatched.`
      );
    }
    ...
    if (!requestId || requestId === 0n) {
      throw new Error(`VRF returned invalid requestId (0) from tx: ${receipt.hash}`);
    }
  ...
  async commitReveal(salt: string): Promise<VRFResult> {
    ...
    if (!commitEvent) {
      throw new Error(
        `Commit-reveal commit succeeded (tx: ${commitReceipt.hash}) but Committed event not found. ` +
        `Verify coordinator address (${this.config.coordinatorAddress}) and ABI.`
      );
    }
    const commitId = contract.interface.parseLog(commitEvent)!.args.commitId;
    if (!commitId || commitId === 0n) {
      throw new Error(`Commit-reveal returned invalid commitId (0) from tx: ${commitReceipt.hash}`);
    }
    ...
    if (!revealEvent) {
      throw new Error(
        `Commit-reveal reveal succeeded (tx: ${revealReceipt.hash}) but Revealed event not found. ` +
        `CommitId: ${commitId}. Verify contract logic.`
      );
    }
    const randomValue = contract.interface.parseLog(revealEvent)!.args.randomValue;
    if (!randomValue || randomValue === 0n) {
      throw new Error(`Commit-reveal returned invalid randomValue (0) for commitId: ${commitId}`);
    }
