...
export async function performSpin(
  ...
  if (!Array.isArray(segments) || segments.length === 0) {
    throw new Error('Segments array must be non-empty');
  }
  for (const seg of segments) {
    if (typeof seg.weight !== 'number' || seg.weight <= 0) {
      throw new Error(`Invalid weight for segment ${seg.id}: ${seg.weight}`);
    }
  }
  ...
  throw new Error(`Spin selection failed: value=${rng.value}, totalWeight=${totalWeight}. This indicates a logic error.`);
}
